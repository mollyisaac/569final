...
extern "C" char *buf_to_file(const uint8_t *buf, size_t size) {
  ...
  strcpy(pathname, "/dev/shm/fuzz-XXXXXX");
  if (pathname == nullptr) {
    return nullptr;
  }
...
//    if (!t1.load("/home/fuzzusers/openafis/data/valid/fvc2002/DB1_B/101_2.iso")) {
    if(!t1.load(path)){
...
//	  std::string path(buf_to_file("12345", 1));


    TemplateISO19794_2_2005<uint32_t, Fingerprint> t1(1);
    if(!t1.load("crash-91cc4b2af5cb165974c72b11da9bfd4acce1bd9c")){
